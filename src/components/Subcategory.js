      ...
      cell: (row) => (
        <div>
          <img
            ...
            src={`https://api.vijayhomeservicebengaluru.in/subcat/${row.subcatimg}`}
            width={"50px"}
            height={"50px"}
          />
        </div>
      ...
      cell: (row) => (
        <div>
          <video width="150" height="150" controls>
            <source
              src={`https://api.vijayhomeservicebengaluru.in/subcat/${row.subcatvideo}`}
              type="video/mp4"
            />
          </video>
        </div>
  ...
  const getServiceByCategory = async () => {
    try {
      let res = await axios.post(
        `https://api.vijayhomeservicebengaluru.in/api/userapp/getservicebycategory/`,
        {
          category,
        }
      );
