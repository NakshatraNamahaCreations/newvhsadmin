...
function Category() {
  ...
  const getcategory = async () => {
    let res = await axios.get(
      "https://api.vijayhomeservicebengaluru.in/api/userapp/getallfeq"
    );
    if ((res.status = 200)) {
  ...
  const columns = [
    ...
    },
    {
      ...
      selector: (row) => (
        <div>
          <img
            src={`https://api.vijayhomeservicebengaluru.in/feq/${row.img}`}
            width="50px"
            height="50px"
          />
        </div>
  ...
  const deletecategory = async (id) => {
    axios({
      method: "post",
      url:
        "https://api.vijayhomeservicebengaluru.in/api/userapp/deletefeq/" + id,
    })
