...
function Services() {
  ...
  useEffect(() => {
    // Fetch data from the backend using your API endpoint with pagination.
    // You can use a library like axios or fetch for making API requests.

    const fetchData = async () => {
      try {
        const response = await fetch(
          `https://api.vijayhomeservicebengaluru.in/api/userapp/getservicespagewise?page=${currentPage}&search=${searchItems}`
        );
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (row) => (
        <div>
          <img
            src={`https://api.vijayhomeservicebengaluru.in/service/${row.serviceImg}`}
            width="50px"
            height="50px"
          />
        </div>
